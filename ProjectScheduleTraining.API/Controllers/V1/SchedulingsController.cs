...
public class SchedulingsController : ControllerBase
{
    ...
    {
        var result = await _mediator.Send(new GetSchedulingsByStudentIdQuery(studentId), cancellationToken);
        return Ok(result);
    }
    ...
    {
        var result = await _mediator.Send(new GetSchedulingsByScheduleIdQuery(scheduleId), cancellationToken);
        return Ok(result);
    }

    /// <summary>
    /// Cria um novo agendamento no sistema.
    /// Acessível por qualquer usuário autenticado (aluno agenda para si mesmo).
    /// </summary>
    [HttpPost]
    [Authorize(Policy = "AllRoles")]
    ...
    /// Restrito a Trainers e Administradores.
    /// </summary>
